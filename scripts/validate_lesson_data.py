...
This validator checks the source-data layer before any public lesson HTML is
built or modified.
...
REQUIRED_PRON_FIELDS = ["focus", "fz", "words", "tip", "prac"]
REQUIRED_PRON_WORD_FIELDS = ["w", "ipa", "bad", "tip"]
...
def validate_list_items(items: Any, fields: list[str], path: str, errors: list[str]) -> None:
    ...
    for index, item in enumerate(items):
        ...
        if not isinstance(item, dict):
            ...
            continue
        require_fields(item, fields, item_path, errors)
    ...
    for index, dialogue in enumerate(dialogues):
        ...
        for line_index, line in enumerate(lines):
            if not (
                ...
                add_error(errors, f"{path}[{line_index}]", "must be [speaker, text] where speaker is A or B")


def validate_pronunciation(pron: Any, errors: list[str]) -> None:
    if not isinstance(pron, dict):
        add_error(errors, "pronunciation", "must be an object")
        return
    require_fields(pron, REQUIRED_PRON_FIELDS, "pronunciation", errors)
    words = pron.get("words")
    validate_list_items(words, REQUIRED_PRON_WORD_FIELDS, "pronunciation.words", errors)


def calculate_counts(data: dict[str, Any]) -> dict[str, int]:
    pron = data.get("pronunciation", {})
    words = pron.get("words", []) if isinstance(pron, dict) else []
    return {
...
def main() -> int:
    ...
    parser.add_argument("--pretty", action="store_true", help="Print pretty JSON report")

...
#
# v4.1 (2026-07-26): fixed a false positive in the gallery-tab-overwrite
# check that incorrectly flagged the standard sibling-gallery architecture
# (empty <div id="tN"></div> followed by a sibling gallery <section>) as a
# CRITICAL violation. Found while validating lessons/travel/u1-l4.html and
# the newly built lessons/travel/u1-l2.html, both of which use this safe,
# already-standard pattern.
...
def check_common_html(path: Path, text: str) -> None:
    ...
    # Gallery tabs must use stable content mounts — but only flag this when the
    # gallery block is actually nested INSIDE a non-empty tab div. In the real
    # production template, `<div class="tpn" id="t4"></div>` is emitted empty,
    # and the gallery <section> is a top-level SIBLING that appears immediately
    # after it, not a child. The renderer's `getElementById('t4').innerHTML = h`
    # only ever replaces the contents of that (empty) div, so the sibling
    # gallery section is never touched — this is safe and must not be flagged.
    # v4.1 (2026-07-26): fixed false positive discovered when validating
    # lessons/travel/u1-l2.html and u1-l4.html, which both use this safe,
    # sibling-gallery architecture but were incorrectly flagged 5x each.
    for n in range(4, 9):
        if not re.search(rf"getElementById\([\"']t{n}[\"']\)\.innerHTML", text):
            continue
        empty_tab_div = re.search(rf'<div class="tpn" id="t{n}">\s*</div>', text)
        if empty_tab_div:
            # Confirmed safe: the tab div itself is empty, so any gallery
            # markup found elsewhere in the document is necessarily a sibling,
            # not a child that the renderer would overwrite.
            continue
